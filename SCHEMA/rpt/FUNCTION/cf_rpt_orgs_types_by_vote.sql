...
BEGIN
	...
	insert into t_results(f_org, f_group, c_org_name, c_boss, c_group, n_count, n_loyal, n_no_loyal, d_date_plan, d_date_fact)
	select 
		u.id as f_org,
		ut.id as f_group,
	...
	CREATE INDEX t_results_f_org_idx ON t_results (f_group);
	CREATE INDEX t_results_f_org_d_date_plan_idx ON t_results (f_group, d_date_plan);
	CREATE INDEX t_results_f_org_d_date_fact_idx ON t_results (f_group, d_date_fact);
...
from (
	select
		...
		(select count(*) from t_results as i where t.f_group = i.f_group) as n_count,
		-- Количество лояльных сотрудников
		sum(t.n_loyal) as n_loyal,
		sf_percent(sum(t.n_loyal), (select count(*) from t_results as i where t.f_group = i.f_group)) as n_loyal_percent,
		-- Количество нелояльных сотрудников
		sum(t.n_no_loyal) as n_no_loyal,
		sf_percent(sum(t.n_no_loyal), (select count(*) from t_results as i where t.f_group = i.f_group)) as n_no_loyal_percent,
		-- Дата голосования 17.09.2021
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_plan = '2021-09-17') as n_plan_day1, -- План (количество)
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_fact = '2021-09-17') as n_fact_day1, -- Факт (количество)
		-- Дата голосования 18.09.2021
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_plan = '2021-09-18') as n_plan_day2, -- План (количество)
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_fact = '2021-09-18') as n_fact_day2, -- Факт (количество)
		-- Дата голосования 19.09.2021
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_plan = '2021-09-19') as n_plan_day3, -- План (количество)
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_fact = '2021-09-19') as n_fact_day3, -- Факт (количество)
		-- за 3 дня
		(select count(*) from t_results as i where t.f_group = i.f_group and i.d_date_fact is not null) as n_plan_all -- Общая явка лояльных сотрудников за 3 дня
	from t_results as t
	group by t.f_group) as m;
END
$$;

ALTER FUNCTION rpt.cf_rpt_orgs_types_by_vote(_f_user integer) OWNER TO vaccine;

COMMENT ON FUNCTION rpt.cf_rpt_orgs_types_by_vote(_f_user integer) IS 'Сводный отчет по отраслям для организации';
